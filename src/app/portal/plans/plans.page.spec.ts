...
describe('PlansPage', () => {
  ...
  async function build(state: Resource<readonly ICarePlanItem[]>): Promise<void> {
    const streams = {
    ...
    streams.carePlan$ = new BehaviorSubject(state) as never;

    await TestBed.configureTestingModule({
      imports: [PlansPage, TranslateModule.forRoot()],
      providers: [
        provideHttpClient(),
  ...
  it('shows skeletons while loading', async () => {
    await build(LOADING as Resource<readonly ICarePlanItem[]>);
