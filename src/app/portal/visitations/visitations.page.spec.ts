...
import { VisitationsPage } from './visitations.page';
...
describe('VisitationsPage', () => {
  let fixture: ComponentFixture<VisitationsPage>;

  async function build(state: Resource<readonly IVisitation[]>): Promise<void> {
    await TestBed.configureTestingModule({
      imports: [VisitationsPage, TranslateModule.forRoot()],
      providers: [
        provideHttpClient(),
        ...
        provideRouter([]),
        {
          ...
          useValue: { visitations$: new BehaviorSubject(state), casesById$: of(LOADING), reload: jest.fn() },
        },
        { provide: PatientContextService, useValue: { careTeamById$: of(new Map()) } },
      ],
    }).compileComponents();

    fixture = TestBed.createComponent(VisitationsPage);
  ...
  it('shows skeletons while loading, and no empty state', async () => {
    await build(LOADING as Resource<readonly IVisitation[]>);
